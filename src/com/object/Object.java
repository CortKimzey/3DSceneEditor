...
public class Object extends BoxElement
{
   ...
   private void readMTL(File file) throws FileNotFoundException {
       Scanner mtlScanner = new Scanner(file);
       Material currentMat = null;
  
       while (mtlScanner.hasNextLine()) {
           String line = mtlScanner.nextLine().trim();
           if (line.isEmpty() || line.startsWith("#")) continue; //comments and empty
           ...
           if (line.startsWith("newmtl ")) {
               ...
               currentMat = new Material(line.substring(7).trim());
           } else if (line.startsWith("Ka ") && currentMat != null) {
               currentMat.Ka = parseColor(line);
           } else if (line.startsWith("Kd ") && currentMat != null) {
               currentMat.Kd = parseColor(line);
           } else if (line.startsWith("Ks ") && currentMat != null) {
               currentMat.Ks = parseColor(line);
           } else if (line.startsWith("Ke ") && currentMat != null) {
               currentMat.Ke = parseColor(line);
           } else if (line.startsWith("Ns ") && currentMat != null) {
               currentMat.Ns = Double.parseDouble(line.substring(3).trim());
           } else if (line.startsWith("Ni ") && currentMat != null) {
               currentMat.Ni = Double.parseDouble(line.substring(3).trim());
           } else if (line.startsWith("d ") && currentMat != null) {
               currentMat.d = Double.parseDouble(line.substring(2).trim());
           } else if (line.startsWith("illum ") && currentMat != null) {
               currentMat.illum = Integer.parseInt(line.substring(6).trim());
           }
       }
   ...
   private double[] parseColor(String line) {
       String[] parts = line.substring(3).trim().split("\\s+");
       double[] color = new double[3];
       for (int i = 0; i < 3 && i < parts.length; i++) {
           color[i] = Double.parseDouble(parts[i]);
       }
       return color;
   }
}
